...
class CasinoBot(SlackBot):
    ...
    async def command_slots(self, user, in_channel, parsed):
        ...
        if bank >= bet:
            ...
            won = bet * calculate_payout_multiple(reels, self.slots_jackpot_symbol)
            if won:
                if won == 30:
                    won = self.slots_jackpot
...
def calculate_payout_multiple(reels, jackpot):
    jacks = reels.count(jackpot)
    if jacks:
        return 30 if jacks > 2 else 4 if jacks == 2 else 1
    else:
        return 10 if reels.count(reels[0]) == 3 else 0
