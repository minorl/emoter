...
class StockBot(SlackBot):
    def __init__(
        ...
        self.available_expr = CaselessLiteral('stockmarket') + Optional(symbols.user_name.setResultsName('stock')) + StringEnd()
        self.available_doc = ('Check prices, inventory and dividends for stocks:\n'
                              '\tstockmarket [user]')

        self.index_name = index_name
        self.index_expr = CaselessLiteral('stockindex') + StringEnd()
        self.index_doc = ('Check the {}:\n'
                          '\tstockindex').format(index_name)

        self.buy_name = 'Buy Stocks'
        self.buy_expr = (CaselessLiteral('buystock') +
                         Optional(symbols.flag_with_arg('amount', Word(nums))) +
                         symbols.user_name.setResultsName('stock'))
        self.buy_doc = 'Buy stocks for {}:\n\tbuystocks <user>'.format(currency_name)

        self.sell_name = 'Sell Stocks'
        self.sell_expr = (CaselessLiteral('sellstock') +
                          Optional(symbols.flag_with_arg('amount', Word(nums))) +
                          symbols.user_name.setResultsName('stock'))
        self.sell_doc = 'Sell stocks for {}:\n\t<user>'.format(currency_name)
    ...
    async def available_stocks(self, user, in_channel, parsed):
        ...
        if 'stock' in parsed:
            try:
                display_stocks = [StockDoc.objects.get(target_user=parsed['stock'])]
            except DoesNotExist:
                return MessageCommand(channel=in_channel, user=user, text='Stock {} does not exist'.format(parsed['stock']))
        else:
            display_stocks = sorted(StockDoc.objects(), key=attrgetter('target_user'))
        ...
        for stock in display_stocks:
            ...
            result.append('*{}* ({} deaths):'.format(stock.target_user.upper(), self.get_deaths(stock.target_user)))
            ...
            result.append('')
        result.append('Note that prices increase as supply goes down. The cost shown is for the next stock purchased or sold.')
        return MessageCommand(
            user=user,
            channel=in_channel,
            text='\n'.join(result))
    ...
    async def buy_stocks(self, user, in_channel, parsed):
        err_message = None
        stock_name = parsed['stock'].lower()
        try:
            stock = StockDoc.objects.get(target_user=stock_name)
        except DoesNotExist:
            err_message = 'Stock not found'
    ...
    def get_deaths(self, target_user):
        return sum(g.deaths for g in get_recent_games(self.name_to_sname[target_user]))
    ...
    async def pay_dividends(self, stock):
        target_user = stock.target_user
        dividend = self.compute_dividend(stock)
        for holdings in StockHoldingsDoc.objects():
            if target_user in holdings.stocks:
                await economy.give(holdings.user, holdings.stocks[target_user] * dividend)
