...
HandlerData = namedtuple('HandlerData', ['name', 'expr', 'channels', 'doc', 'priority', 'admin'])
...
def register(name=None, expr=None, channels=None, doc=None, priority=0, admin=False):
    """
    ...
    def wrap(f):
        ...
        return wraps(f)(SlackHandler(f, HandlerData(name, expr, channels, doc, priority, admin)))
    return wrap
        ...
        def new_init(self, *args, **kwargs):
            ...
            for name in names:
                ...
                mapped_data = HandlerData(
                    name=name, expr=expr, channels=channels, doc=doc, priority=data.priority, admin=data.admin)
